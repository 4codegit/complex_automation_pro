...
const OUT = "/home/narziev/Documents/AutoPro/docs/Презентация_Новая.pptx";
...
pptx.layout = "WIDE";
pptx.author = "CAP";
pptx.title = "CAP — Новая презентация";
...
bullets(s, [
     ...
     "Эти показатели представляют макромайн‑анализ (агрегированные характеристики процесса) и основаны на данных‑майнинге (data‑main) – обработке больших объёмов телеметрии для выявления закономерностей и оптимизации параметров обогащения"
...
bullets(s, [
      ...
      "Реализация ПИД-алгоритмов возможна на сервере capd, внешних ПЛК или ESP32 через Modbus FC6; локальные контуры на ESP32 обеспечивают быструю обратную связь",
    ], 9.35, 1.7, 3.45, { size: 12.5, gap: 9 });
}
...
// ---------- 14. Инновационные алгоритмы металлургического баланса ----------
{
  const s = baseSlide(14, "Инновационные алгоритмы металлургического баланса", "алгоритмы");
  const h = shot(s, `${A}/metallurgy.png`, 0.8, 1.45, 8.3);
  bullets(s, [
    "Двухпродуктовый баланс с формулой ε = β(α−θ)/(α(β−θ))×100",
    "Встроенный детектор несогласованности: невязка ≤ ±1 % и сходимость извлечений ≤ 5 %",
    "Автокоррекция при дрейфе XRF: адаптивный коэффициент калибровки",
    "Индикатор закрытия баланса в реальном времени на мнемосхеме",
    "Каждый показатель сопровождается строкой формулы и тегами‑источниками (аудируемость)",
  ], 9.35, 1.7, 3.45, { size: 12.5, gap: 9 });
}

// ---------- 15. Адаптивная настройка ПИД-контуров ----------
{
  const s = baseSlide(15, "Адаптивная настройка ПИД-контуров", "управление");
  shot(s, `${A}/control.png`, 0.8, 1.45, 8.3);
bullets(s, [
      "Састройка коэффициентов Kp/Ki на основе оценки процесса gain",
      "Gain scheduling в зависимости от производительности питателя",
      "Безбамперный переход в АВТО: инициализация интеграла от фактической позиции",
      "Анти‑windup и защита от насыщения актуатора",
      "Watchdog 10 с: при потере PV — переход в РУЧН, замораживание выхода",
      "Алгоритмы адаптивной настройки могут выполняться на сервере capd, внешних ПЛК или ESP32, обеспечивая гибкость распределённого управления",
    ], 9.35, 1.7, 3.45, { size: 12.5, gap: 9 });
}

// ---------- 16. Метод рационализации тревог по ISA-18.2 ----------
// ---------- 16. Автоматическое регулирование pH без вмешательства оператора ----------
{
  const s = baseSlide(16, "Автоматическое регулирование pH без вмешательства оператора", "авторегуляция");
  shot(s, `${A}/control.png`, 0.8, 1.45, 8.3);
  bullets(s, [
    "Контур pH поддерживает заданный диапазон 10,0–10,5 через подачу реагентов (щелочь/кислота) по сигналу ПИД.",
    "Алгоритм самонастройки: адаптивный gain scheduling в зависимости от температуры и скорости потока.",
    "Безбамперный переход в АВТО и анти‑windup защита исключают скачки при переключении режимов.",
    "При отклонении pH более ±0,2 от уставки формируется предупреждение, а при более ±0,5 — критическая тревога с автоматическим переходом в безопасный режим.",
    "Все действия логируются в аудит с меткой времени и оператором (если вмешательство требуется).",
  ], 9.35, 1.7, 3.45, { size: 12.5, gap: 9 });
}

// ---------- 17. Метод рационализации тревог по ISA-18.2 ----------
{
  const s = baseSlide(17, "Метод рационализации тревог по ISA-18.2", "тревоги");
  shot(s, `${A}/alarms.png`, 0.8, 1.45, 8.3);
  bullets(s, [
    "Алгоритмическое определение пределов lo/hi на основе стандартного отклонения",
    "Гистерезис 1 % шкалы и задержка срабатывания 3 с (критические) / 10 с",
    "Автоматическое присваивание приоритетов в зависимости от влияния на продукт",
    "Неизменяемый журнал событий: сработала/сброшена/квитирована с оператором",
    "Квитирование от имени оператора с обязательным комментарием",
  ], 9.35, 1.7, 3.45, { size: 12.5, gap: 9 });
}

// ---------- 18. Надежная доставка телеметрии и аудит ----------
{
  const s = baseSlide(18, "Надежная доставка телеметрии и аудит", "надежность");
  shot(s, `${A}/trends.png`, 0.8, 1.45, 8.3);
  bullets(s, [
    "Store‑and‑forward буфер SQLite с message_id для идемпотентной доставки",
    "Доставка «exactly‑once» без дублей и сохранением порядка при восстановлении связи",
    "Криптографически защищённый аудит: каждая мутация подписана сессией и меткой времени",
    "Разделение ролей: оператор — квитирование и уставки; администратор — реестр и аудит",
    "Машинная аутентификация шлюзов по постоянному времени сравнения секрета",
  ], 9.35, 1.7, 3.45, { size: 12.5, gap: 9 });
}

// ---------- 19. Характеристики и статус проекта ----------
{
  const s = baseSlide(19, "Характеристики и статус проекта", "итоги");
  chips(s, [
